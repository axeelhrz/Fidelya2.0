...
class NotificationService {
  ...
  /**
   * Build recipients list based on filter
   */
  async buildRecipients(
    asociacionId: string,
    filter: 'todos' | 'al_dia' | 'vencidos' | 'elegir',
    selectedIds: string[] = []
  ): Promise<string[]> {
    try {
      if (filter === 'elegir') return selectedIds;

      // Firestore lightweight fetch to resolve socio IDs
      const { collection, query, where, getDocs } = await import('firebase/firestore');
      const { db } = await import('@/lib/firebase');

      const sociosCol = collection(db, 'socios');

      const constraints = [where('asociacionId', '==', asociacionId)];
      if (filter === 'al_dia') constraints.push(where('estadoMembresia', '==', 'al_dia'));
      if (filter === 'vencidos') constraints.push(where('estadoMembresia', 'in', ['vencido', 'atraso']));

      const q = query(sociosCol, ...constraints);
      const snap = await getDocs(q);
      return snap.docs.map(d => d.id);
    } catch (error) {
      console.error('Error building recipients:', error);
      return [];
    }
  }
