...
export interface QueuedNotification {
  ...
  // REMOVED: priority - Las notificaciones se procesan por orden de ejecución (FIFO)
  attempts: number;
  maxAttempts: number;
...
class NotificationQueueService {
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      // Las notificaciones se procesan por orden de ejecución (FIFO - First In, First Out)
      const pendingQuery = query(
        collection(db, this.QUEUE_COLLECTION),
        where('status', '==', 'pending'),
        where('processAfter', '<=', Timestamp.fromDate(now)),
        orderBy('processAfter', 'asc'), // Orden de ejecución: primero las más antiguas
