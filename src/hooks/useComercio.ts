...
export const useComercio = () => {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      console.log('[Get Comercio Stats] Iniciando carga de estadísticas para:', comercioId);
      const statsData = await comercioService.getComercioStats(comercioId);
      setStats(statsData);
      console.log('[Get Comercio Stats] Estadísticas cargadas exitosamente:', statsData);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Error al cargar estadísticas';
      console.error('[Get Comercio Stats] Error:', err);
      
      // Mostrar un mensaje de error más específico al usuario
      if (errorMessage.includes('Comercio no encontrado')) {
        toast.error('Tu cuenta de comercio no se encuentra correctamente configurada. Por favor, contacta al administrador.');
      } else if (errorMessage.includes('problema de sincronización')) {
        toast.error('Hay un problema de sincronización con tu cuenta. Por favor, contacta al soporte técnico.');
      } else {
        toast.error('Error al cargar las estadísticas del comercio');
      }
