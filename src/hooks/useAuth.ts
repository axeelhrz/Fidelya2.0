...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  // Enhanced function to check email verification status with temporary authentication
  const checkEmailVerification = async (email?: string, password?: string): Promise<boolean> => {
    console.log('🔐 Checking email verification status...', { 
      hasFirebaseUser: !!firebaseUser, 
      email: email || firebaseUser?.email 
    });

    // If we have an active firebase user, check directly
    if (firebaseUser) {
      try {
        await reload(firebaseUser);
        const verified = firebaseUser.emailVerified;
        setIsEmailVerified(verified);
        console.log('🔐 Direct check result:', verified);
        return verified;
      } catch (error) {
        console.error('🔐 Error checking email verification directly:', error);
      }
    }

    // If no active user but we have email and password, try temporary authentication
    if (email && password) {
      console.log('🔐 Attempting temporary authentication to check verification...');
      try {
        const response = await authService.checkEmailVerificationWithAuth(email, password);
        console.log('🔐 Temporary auth check result:', response);
        return response.isVerified || false;
      } catch (error) {
        console.error('🔐 Error in temporary auth check:', error);
        return false;
      }
    }

    console.log('🔐 Cannot check verification: no active user and no credentials provided');
    return false;
  };
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      try {
        ...
        if (firebaseUser) {
          ...
          // Always reload user to get the latest verification status
          await reload(firebaseUser);
...
export function useEmailVerification() {
  ...
  return {
    ...
    resendVerification: (password?: string, email?: string) => {
      // Use provided email or fallback to firebaseUser email
      const targetEmail = email || firebaseUser?.email;
      if (targetEmail) {
        return resendEmailVerification(targetEmail, password);
      }
      return Promise.resolve({ success: false, error: 'No se encontró el email para reenviar la verificación' });
    }
  };
}