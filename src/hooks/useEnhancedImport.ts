...
import { validateAndFormatPhone, formatPhoneForArgentina } from '@/utils/phone-validator';
...
export const useEnhancedImport = () => {
  ...
  const validateRow = useCallback((row: Record<string, string>, rowIndex: number, columnMappings: ColumnMapping[]): ValidationError[] => {
            ...
            if (value) {
              ...
              if (!phoneResult.isValid) {
                errors.push({
                  ...
                  field: mapping.targetField,
                  value,
                  error: phoneResult.error || 'Formato de teléfono inválido para WhatsApp',
                  severity: 'warning',
                  fixable: true
                });
              }
    ...
    return errors;
  }, [validateEmail, validateDNI, validateDate]);
        ...
        if (mapping.csvColumn && row[mapping.csvColumn]) {
          ...
          switch (mapping.targetField) {
            ...
            case 'telefono':
              ...
              if (value) {
                value = formatPhoneForArgentina(value);
              }
              break;
            case 'dni':
