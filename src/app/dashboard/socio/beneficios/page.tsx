...
export default function SocioBeneficiosPage() {
  ...
  // Función para refrescar datos después de una validación exitosa
  const handleValidationSuccess = async () => {
    // Refrescar tanto los beneficios como el historial
    await Promise.all([
      refrescar(),
      refrescarHistorialDespuesValidacion()
    ]);
  ...
  // Mantener la función original para compatibilidad con otros componentes
  const handleUseBenefit = async () => {
    try {
      // Esta función ya no se usa para socios, pero la mantenemos por compatibilidad
      console.warn('handleUseBenefit está deprecated para socios, usar validación en su lugar');
    } catch (error) {
