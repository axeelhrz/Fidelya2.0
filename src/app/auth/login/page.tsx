...
function LoginPageContent() {
  ...
  const handleLogin = async (data: LoginFormData) => {
    try {
      ...
      // Check if we should redirect to returnUrl (only on mobile and for benefit validation)
      const returnUrl = searchParams.get('returnUrl');
      
      if (isMobile && returnUrl) {
        try {
          const decodedUrl = decodeURIComponent(returnUrl);
          // Only redirect to returnUrl if it's a benefit validation page
          if (decodedUrl.includes('/validar-beneficio')) {
            console.log('🔐 Mobile device detected, redirecting to benefit validation:', decodedUrl);
  ...
  // Enhanced function to handle resend verification from login page
  // (Removed unused handleResendFromLogin function)
