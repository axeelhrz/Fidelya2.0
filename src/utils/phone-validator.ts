/**
 * Phone Validator and Formatter for Multiple Countries
 * Supports WhatsApp numbers from any country
 * 
 * Supported formats:
 * - International: +[country_code][number]
 * - With country code: [country_code][number]
 * - Argentina local: 1112345678, 91112345678, etc.
 * 
 * Examples:
 * - Argentina: +5491112345678, 1112345678
 * - Uruguay: +59892388748
 * - Chile: +56912345678
 ...
 * 
 * Validation rules:
 * 1. Must start with + or have a valid country code
 * 2. Must have 7-15 digits (international standard)
 * 3. Must be a valid WhatsApp number
 */
export function validateAndFormatPhoneInternational(phone: string): PhoneValidationResult {
  const original = phone;
  ...
  // Must have 7-15 digits (international standard for phone numbers)
  if (digitsOnly.length < 7 || digitsOnly.length > 15) {
    return {
      isValid: false,
      formatted: '',
      original,
      error: `El número debe tener entre 7 y 15 dígitos. Ingresaste ${digitsOnly.length} dígitos`
    };
  }
  ...
  // Try to find the country code (check 3-digit, then 2-digit, then 1-digit)
  if (countryCodes[digitsOnly.substring(0, 3)]) {
    countryCode = digitsOnly.substring(0, 3);
    country = countryCodes[countryCode];
  } else if (countryCodes[digitsOnly.substring(0, 2)]) {
    countryCode = digitsOnly.substring(0, 2);
    country = countryCodes[countryCode];
  } else if (countryCodes[digitsOnly.substring(0, 1)]) {
