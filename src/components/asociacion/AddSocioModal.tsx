...
const AddSocioModal = memo<AddSocioModalProps>(({ 
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (formData.telefono && formData.telefono.trim() !== '+54') {
      // Validate Argentine phone format: +54 followed by area code and number
      if (!/^\+54\s*\d{1,4}\s*\d{6,8}$/.test(formData.telefono.replace(/[\s\-\(\)]/g, ' ').replace(/\s+/g, ' '))) {
        newErrors.telefono = 'Formato: +54 [código área] [número] (ej: +54 11 12345678)';
      }
    }
  ...
  const handleInputChange = useCallback((field: keyof SocioFormData, value: string | number) => {
    // Special handling for phone field to maintain +54 prefix
