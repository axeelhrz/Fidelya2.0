...
export const EmailVerification: React.FC<EmailVerificationProps> = ({ 
  ...
  // Fixed function to handle resend verification
  const handleResendVerification = async () => {
    ...
    try {
      ...
      // Call the resend function without password first, using the displayEmail
      const response = await resendVerification(undefined, displayEmail);
      ...
      if (response.success) {
        ...
        // If the error indicates that password is needed, show the password field
        if (response.error?.includes('contraseña') || response.error?.includes('password') || response.error?.includes('proporcionar')) {
  ...
  // Fixed function to handle resend with password
  const handleResendWithPassword = async () => {
    ...
    try {
      ...
      // Call the resend function with password, using the displayEmail
      const response = await resendVerification(password, displayEmail);
  ...
  const handleCheckVerification = async () => {
    ...
    try {
      console.log('🔐 Checking verification status...', { displayEmail, hasPassword: !!password });
      
      // If we have a password (from the password field), use it for verification
      let isVerified = false;
      
      if (password && password.trim()) {
        // Use the enhanced verification with credentials
        isVerified = await checkVerification();
      } else {
        // Try without credentials first
        isVerified = await checkVerification();
        
        // If that fails and we don't have password, show password field
        if (!isVerified && !showPasswordField) {
          setShowPasswordField(true);
          toast.error('Necesitas proporcionar tu contraseña para verificar el estado del email');
          return;
        }
      }
      ...
      if (isVerified) {
        ...
        toast.error('El email aún no está verificado. Revisa tu bandeja de entrada o verifica el enlace.');
      }
    } catch (error) {
