...
export const BeneficioCard: React.FC<BeneficioCardProps> = ({
  ...
  const handleUse = async () => {
    // Cambiar para usar la función de validación en lugar de la función de uso
    if (userRole === 'socio') {
      setLoading(true);
      try {
        const result = await validarBeneficio(beneficio.id, beneficio.comercioId);
        if (result.success) {
        ...
        console.error('Error validando beneficio:', error);
      } finally {
        setLoading(false);
      }
    } else if (onUse) {
      // Para otros roles, usar la función original
      setLoading(true);
      try {
        ...
        <div className={`p-6 ${view === 'list' ? 'flex-1' : ''}`}>
          ...
          {showActions && (
            <div className="flex gap-2">
              ...
              {userRole === 'socio' && beneficio.estado === 'activo' && (
                <Button
                  size="sm"
